...
画像はQualtricsの質問領域とブラウザの表示領域の両方に収まるよう、レスポンシブに縮小されます。判断刺激は縦横比を維持して最大`84vw × 64vh`、宇宙人・導入画像は説明文、ページ番号、操作ボタンまで一画面に収まりやすいよう最大`78vw × 46vh`です。実験領域をブラウザ幅まで強制的に拡張せず、横方向のはみ出しを防ぎます。この表示上の調整は、400 msの提示時間や刺激画像そのものには影響しません。
