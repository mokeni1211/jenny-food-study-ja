...
画像はブラウザの表示領域に応じてレスポンシブに拡大されます。判断刺激は縦横比を維持して最大`96vw × 82vh`、宇宙人・導入画像は説明文とボタンの領域を残しながら最大`94vw × 68vh`です。Qualtricsテーマ側の通常の質問幅に制限されないよう、実験領域は画面幅を基準に配置します。この表示上の拡大は、400 msの提示時間や刺激画像そのものには影響しません。
