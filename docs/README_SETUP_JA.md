...
画像はブラウザの表示領域に応じてレスポンシブに縮小されます。判断試行だけは幅`94vw`（上限1200px）、高さ72vhまで広げ、Qualtricsの親領域から左右均等に張り出すよう中央配置します。張り出した部分を切り取る`overflow: hidden`は使用しません。宇宙人・導入画像は通常の実験ページ内で横幅の最大100%かつ高さ54vhまで表示します。この表示上の調整は、400 msの提示時間や刺激画像そのものには影響しません。
