...
画像はQualtricsの質問領域とブラウザの表示領域の両方に収まるよう、レスポンシブに縮小されます。実験ページの親領域を最大1000pxとし、判断刺激はその横幅の最大100%かつ高さ68vhまで、宇宙人・導入画像は横幅の最大100%かつ高さ54vhまで表示します。判断刺激を画面幅まで強制的に拡張せず、縦横のはみ出しを防ぎます。この表示上の調整は、400 msの提示時間や刺激画像そのものには影響しません。
