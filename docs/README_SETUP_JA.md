...
画像はブラウザの表示領域に応じてレスポンシブに縮小されます。判断試行だけはQualtricsの通常質問幅から独立した中央配置領域を使用し、画面幅の96%（上限1200px）かつ高さ72vhまで表示します。宇宙人・導入画像は通常の実験ページ内で横幅の最大100%かつ高さ54vhまで表示します。判断領域は画面幅の96%を超えないため、横方向のはみ出しを防ぎます。この表示上の調整は、400 msの提示時間や刺激画像そのものには影響しません。
